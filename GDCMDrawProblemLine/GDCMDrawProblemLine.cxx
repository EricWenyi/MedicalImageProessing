...
int main( int argc, char* argv[] ){
	...
	for( inIterator.GoToBegin(); !inIterator.IsAtEnd(); inIterator.NextSlice() ){
		...
		for (int i = 0; i< contours.size(); i++){
			...
			printf( "%d Distance calculation OK\n", i + 1 );

			for( int j = 0; j < contours[i].size(); j++ ){
				for( int k = 0; k < contours[i].size(); k++ ){ 
					if( disPixel[j][k] < disCounterPixel[j][k] ){
						float disOfJAndK = (float)sqrt( (double)((contours[i][j].x - contours[i][k].x) * (contours[i][j].x - contours[i][k].x) + (contours[i][j].y - contours[i][k].y) * (contours[i][j].y - contours[i][k].y)) );
						if( disPixel[j][k] / disOfJAndK > 8 ){		
							if( k > j ){
								for( int p = j; p <= k; p++ ){
