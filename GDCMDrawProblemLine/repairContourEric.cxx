...
int main( int argc, char* argv[] ){
	...
	for( inIterator.GoToBegin(); !inIterator.IsAtEnd(); inIterator.NextSlice() ){
		...
		for (int i = 0; i< contours.size(); i++){//一般情况下contours.size() == 2
			...
			if(tempRBS.size() < 60)//如果图像过小，则不能修复
				continue;
			...
			if(left_or_right(tempRBS) == 0){//如果left_or_right为0代表是左肺
				for(int i = 0; i < (int)(PERCENTOFPOINTS_F * tempRBS.size()) ; i++){
					...
					for(int j = LOWERBOUND; j < UPPERBOUND; j++){
						...
						float Con_dis = 0.0;
						//计算欧式距离
						Eu_dis = Euc_distance(tempRBS[i].x,tempRBS[i].y,tempRBS[(i+j)%m].x,tempRBS[(i+j)%m].y);

						//计算contour距离
						for(int q = 0; q < j ; q++){
							int p = i;
							Con_dis += Euc_distance(tempRBS[p%m].x,tempRBS[p%m].y,tempRBS[(p+1)%m].x,tempRBS[(p+1)%m].y);
							p++;
						}
						//如果到达临界值
		...
		for( int i = 0; i < contours.size(); i++ ){
			drawContours( drawing, contours, i, Scalar( 255 ), CV_FILLED, 8, hierarchy, 0, Point(0, 0) );
		}
