...
int main( int argc, char * argv[] ){
	...
	for( iterator.GoToBegin(); !iterator.IsAtEnd(); iterator.NextSlice() ){
		...
		typedef itk::ScalarImageKmeansImageFilter< ImageType2D, ImageType2D > kmeansFilterType;
		kmeansFilterType::Pointer kmeansFilter = kmeansFilterType::New();
		kmeansFilter->SetInput( extractFilter->GetOutput() );
		kmeansFilter->SetUseNonContiguousLabels( false );
		kmeansFilter->AddClassWithInitialMean( 0.0 );
		kmeansFilter->AddClassWithInitialMean( 0.0 );
		
		try{
			kmeansFilter->Update();
		} catch ( itk::ExceptionObject & err ){
			std::cerr << "ExceptionObject kmeansFilter->Update() caught !" << std::endl;
			std::cerr << err << std::endl;
			return EXIT_FAILURE;
		}

		//设定图片组合器的输入
		joinSeries->PushBackInput( kmeansFilter->GetOutput() );
	}
